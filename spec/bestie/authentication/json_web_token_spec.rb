RSpec.describe Bestie::Authentication::JSONWebToken do
  let(:user) { create(:user) }
  let(:exp) { 24.hours.from_now }
  let(:jwt) { Bestie::Authentication::JSONWebToken.encode({ user_id: user.id }, exp) }

  describe 'decode' do
    subject { Bestie::Authentication::JSONWebToken.decode(jwt) }
  ...
  describe 'encode' do
    subject { Bestie::Authentication::JSONWebToken.encode({ user_id: user.id }, exp) }

    context 'works ok' do
      it { expect { subject }.not_to raise_error }
      it do
        token = Bestie::Authentication::JSONWebToken.decode(subject)
